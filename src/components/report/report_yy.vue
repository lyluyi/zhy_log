    ...
    <div class="com_report_yy_inputGroup">
      <Row :gutter="16" class="mb10">
        ...
        <Col class="col_flex" span="8">
          ...
          <!-- <DatePicker :options="limitDateA" type="year" placeholder="Select year" style="width: 200px" v-model="allData.time" @on-change="allData.time=$event"></DatePicker> -->
      ...
      <Row :gutter="16" class="mt20 mb20">
        <Col class="col_flex tr" span="24">
          <Button type="success" size="large" style="margin:auto;width:128px;" @click="exportTable">一键年结</Button>
        </Col>
      </Row>
    </div>
...
import { getComReportYy, getReportYyGenerate } from '@/server/api'
...
export default {
  ...
  methods: {
    ...
    exportTable () {
      ...
      this.$Spin.show()
      let params = this.allData
      getReportYyGenerate(params).then((res) => {
        this.$Spin.hide()
        this.$Message.success('一键年结成功！')
      }).catch((err) => {
        this.$Spin.hide()
        this.$Message.error('数据操作异常！')
        throw err
