<template>
  <div class="group_report_yy">
    <div class="group_report_yy_title mb20">
      集团年度报表
    </div>
    <div class="group_report_yy_inputGroup">
...
export default {
  ...
  methods: {
    ...
    exportTable () {
      let params = this.allData
      this.$axios.post('user/report/jt/year/exportUserCdChangeInfo', params, {
        responseType: 'blob',
        timeout: 60000 * 2
      }).then(res => {
        let blob = res.data
...
.group_report_yy .ivu-table td, .ivu-table th {
  text-align: center !important;
}
.group_report_yy_title{
  background: #2d8cf0;
  width: 100%;
  text-align: left;
  color: #fff;
...
.group_report_yy .ivu-table-cell {
