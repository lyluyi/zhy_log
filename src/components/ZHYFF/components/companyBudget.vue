  ...
  created () {
    ...
    this.$axios.post(url + 'BudgetUsage/selectC0001.do', params).then(res => {
      if (res.data.success === true || res.data.success === 'true') {
        ...
        this.data6 = res.data.budgetUsageList
        this.listLength = res.data.budgetUsageListSize
      } else {
    ...
    this.$axios.post(url + 'Correspond/selectUtilYear.do', {}).then(res => {
      this.$Spin.hide()
      if (res.data.success === true || res.data.success === 'true') {
        this.yearType = res.data.correspondList
      } else {
        this.$Message.error('年份更新异常！')
      }
    }).catch(err => {
      this.$Message.error('年份更新失败！')
      throw err
    })
